...
export async function getBlogPostData(params, locale, preview) {
  const slug = params.slug[1];

  const blogPostsData = await fetchAPI(
    `/blog-posts?slug=${slug}&_locale=${locale}&status=published${
      preview ? "&status=draft" : ""
    }`
  );

  if (blogPostsData == null || blogPostsData.length === 0) {
    return null;
  }
